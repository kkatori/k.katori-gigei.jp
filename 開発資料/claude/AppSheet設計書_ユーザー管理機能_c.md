...
| `family_name` | Text | | 姓（苗字）※GWS Last Name に対応 |
| `given_name` | Text | | 名（名前）※GWS First Name に対応 |
...
Google同期、rakumoコンタクト更新、およびGoogleアカウント登録（予約実行）の定期実行スケジュールを設定する画面。

**定期実行対象：**

| # | 対象 | 説明 |
|---|------|------|
| 1 | Google同期 | Cloud SQLとGWSの同期を定期実行 |
| 2 | rakumoコンタクト更新 | Google同期の30分後に自動実行 |
| 3 | Googleアカウント登録（予約実行） | ユーザーごとに設定された予約日時にGWSアカウントを登録 |

**入力項目：**

| 項目 | AppSheet型 | 内容 |
|------|-----------|------|
| 実行時刻（Google同期） | Enum | 0〜23時を1時間単位で選択（例：6→06:00実行） |
| アカウント登録予約日時 | DateTime | Googleアカウント登録を実行する日時（YYYY/MM/DD hh形式、JST、1時間単位）。ユーザーごとに個別設定可能 |
...
- Googleアカウント登録の予約実行はユーザーごとに個別スケジュールで実行される旨の説明
...
> 設定値はCloud SQLの設定テーブル（AppSheetから変更可能）に格納される。保存ボタン押下でWebhook経由でCloud Runに送信し、settingsテーブルを更新する。ローテーションは毎日AM 1:00 JST（AppSheet設定画面から変更可能）にCloud Schedulerジョブで自動実行される。

**ログ出力レベル設定：**

同一画面内にログ出力レベルの設定セクションを設ける。

| 項目 | AppSheet型 | 内容 | デフォルト値 |
|------|-----------|------|------------|
| ログ出力レベル | Enum | サマリ（主要操作のみ記録）/ 詳細（全処理ステップを記録）を選択 | サマリ |

> 設定値はCloud SQLの設定テーブル（setting_key: `log_output_level`）に格納される。AppSheetのビュー（設定画面）からサマリ/詳細を選択可能。保存ボタン押下でWebhook経由でCloud Runに送信する。
...
| 重複チェック | AppSheet入力時の重複チェック：行わない。重複チェックはCloud Run処理時（GWS API呼び出し時）に実施する |
| エラー時の動作 | Cloud Runからのレスポンスでユーザーにエラーメッセージを表示する |
| 注意事項 | 既存メールアドレスの場合、GWS APIがエラーを返し、Cloud Runがエラーレスポンスを返却。AppSheetがエラーメッセージとしてユーザーに通知する |
