...
| | v2.2：最終版PDF（ver1.1 2026/04/20更新）の要件確定事項を反映 — 日時指定・表示方法のJST化（4.8節・13章追加）、予定実行の指定方法追加（4.8節）、性能要件の数値具体化（7章）、Googleアカウントの定期実行追加（4.2節）、グループ種別にメーリングリストを追記（4.3節・8.1節）、重複登録時のエラーハンドリング詳細化（7章）、運用手順の詳細を追記（8章） |
| | v2.3：チェック指摘事項の修正 — First Name/Last Name説明修正（M-1）、定期/予約実行対象にGoogleアカウント登録追加（M-2）、メーリングリスト用デフォルト値追加（M-3）、AppSheet操作用語統一（M-4）、ユーザーCSVにGoogle属性追加（M-5）、rakumoライセンスデフォルト値明記（NG-1）、バッチAPIリクエスト方式明記（C-1）、予定実行日付形式確定（C-2）、ログ出力レベル設定方法確定（C-3）、GWSグループ物理削除明記（C-4）、設定テーブル方式確定（C-5）、ログローテーション時刻確定（C-6）、重複チェックタイミング確定（C-7） |
...
| グループ種別 | 「rakumo用」「ドライブ用」「メーリングリスト用」の選択により、デフォルト値を自動設定 |
...
AppSheet 個別操作時は、AppSheet アクション（Webhook）経由で Cloud Run API を呼び出し、Cloud SQL および Google Workspace をリアルタイムに更新する。
...
| 定期実行 | 設定テーブルで実行方法を制御（詳細は4.8節参照） |

> 即時実行と定期実行の切り替えはCloud SQLの設定テーブルで制御する。設定テーブルはAppSheetから参照・変更が可能である。
...
| 7 | First Name | string | ユーザーの名（名前） | |
| 8 | Last Name | string | ユーザーの姓（苗字） | |
...
| 7 | First Name | string | 入力可 | ― | ユーザーの名（名前） |
| 8 | Last Name | string | 入力可 | ― | ユーザーの姓（苗字） |
...
| 4 | First Name | 必須 | ― | ユーザーの名（名前） |
| 5 | Last Name | 必須 | ― | ユーザーの姓（苗字） |
...
本機能が管理するグループは以下の3種別とする。同時更新が可能であり、複数種別のグループをまとめて処理できる。

| グループ種別 | プレフィックス | 例 | 用途 |
|------------|-------------|-----|------|
| Googleドライブ用 | なし（命名規則なし） | ― | Googleドライブのアクセス権限管理 |
| メーリングリスト用 | `ml-` | `ml-xxxx@so.tohoyk.co.jp` | メーリングリスト |
| rakumo用 | `rk-` | `rk-10000@so.tohoyk.co.jp` | rakumoサービスの利用グループ（階層構造） |

> rakumo用グループは組織階層構造を持つ。ドライブ用・メーリングリスト用はフラット構造。グループ削除後に新規作成するとGoogleドライブのアクセス権を再設定する必要があるため、名称変更を推奨する。
...
グループ作成・変更時に入力可能な項目とデフォルト値の一覧。グループ種別（rakumo用／ドライブ用／メーリングリスト用）により一部デフォルト値が異なる。

| # | プロパティ名 | 型 | 入力/読取専用 | デフォルト値（rakumo用） | デフォルト値（ドライブ用） | デフォルト値（メーリングリスト用） |
|---|------------|-----|-----------|----------------------|------------------------|----------------------------------|
| 1 | GroupID | string | 読取専用 | ― | ― | ― |
| 2 | email | string | 入力可 | ― | ― | ― |
| 3 | name | string | 入力可 | ― | ― | ― |
| 4 | description | string | 入力可 | ― | ― | ― |
| 5 | whoCanJoin | string | 入力可 | `INVITED_CAN_JOIN` | `INVITED_CAN_JOIN` | `INVITED_CAN_JOIN` |
| 6 | whoCanViewMembership | string | 入力可 | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` |
| 7 | whoCanViewGroup | string | 入力可 | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` |
| 8 | whoCanPostMessage | string | 入力可 | `NONE_CAN_POST` | `ALL_IN_DOMAIN_CAN_POST` | `ALL_IN_DOMAIN_CAN_POST` |
| 9 | allowWebPosting | string | 入力可 | `FALSE` | `FALSE` | `FALSE` |
| 10 | isArchived | string | 入力可 | `TRUE` | `TRUE` | `TRUE` |
| 11 | membersCanPostAsTheGroup | string | 入力可 | `TRUE` | `TRUE` | `TRUE` |
| 12 | includeInGlobalAddressList | string | 入力可 | `FALSE` | `TRUE` | `TRUE` |
| 13 | whoCanLeaveGroup | string | 入力可 | `NONE_CAN_LEAVE` | `NONE_CAN_LEAVE` | `NONE_CAN_LEAVE` |
| 14 | whoCanModerateMembers | string | 入力可 | `NONE` | `NONE` | `NONE` |
| 15 | whoCanModerateContent | string | 入力可 | `NONE` | `NONE` | `NONE` |
| 16 | whoCanAssistContent | string | 入力可 | `NONE` | `NONE` | `NONE` |
| 17 | enableCollaborativeInbox | string | 入力可 | `FALSE` | `FALSE` | `FALSE` |
| 18 | whoCanDiscoverGroup | string | 入力可 | `ALL_MEMBERS_CAN_DISCOVER` | `ALL_MEMBERS_CAN_DISCOVER` | `ALL_MEMBERS_CAN_DISCOVER` |
| 19 | defaultSender | string | 入力可 | 空白 | 空白 | 空白 |
...
AppSheetフォームからグループを作成する際の入力フィールドとデフォルト値。Google管理コンソールは使用せず、すべてAppSheetから操作する。グループ種別（rakumo用／ドライブ用／メーリングリスト用）によりデフォルト値が一部異なる。

| # | フィールド名 | 入力 | デフォルト値（rakumo用） | デフォルト値（ドライブ用） | デフォルト値（メーリングリスト用） | 備考 |
|---|-----------|------|----------------------|------------------------|----------------------------------|------|
| 1 | GroupID | 自動 | システム採番（G100000形式） | 同左 | 同左 | ユーザー入力不可 |
| 2 | email | 必須 | ― | ― | ― | rakumo用: `rk-番号@so.tohoyk.co.jp`、メーリングリスト用: `ml-xxxx@so.tohoyk.co.jp`、ドライブ用: 命名規則なし |
| 3 | name | 必須 | ― | ― | ― | グループの表示名（最大75文字） |
| 4 | description | 任意 | ― | ― | ― | グループの説明（最大4,096文字） |
| 5 | whoCanJoin | 任意 | `INVITED_CAN_JOIN` | `INVITED_CAN_JOIN` | `INVITED_CAN_JOIN` | 招待されたユーザーのみ参加可 |
| 6 | whoCanViewMembership | 任意 | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` | |
| 7 | whoCanViewGroup | 任意 | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` | `ALL_MEMBERS_CAN_VIEW` | |
| 8 | whoCanPostMessage | 任意 | `NONE_CAN_POST` | `ALL_IN_DOMAIN_CAN_POST` | `ALL_IN_DOMAIN_CAN_POST` | rakumo用はメール送信不可 |
| 9 | allowWebPosting | 任意 | `FALSE` | `FALSE` | `FALSE` | Webからの投稿不可 |
| 10 | isArchived | 任意 | `TRUE` | `TRUE` | `TRUE` | メッセージをアーカイブする |
| 11 | membersCanPostAsTheGroup | 任意 | `TRUE` | `TRUE` | `TRUE` | グループアドレスでの送信を許可 |
| 12 | includeInGlobalAddressList | 任意 | `FALSE` | `TRUE` | `TRUE` | rakumo用はGALに非表示 |
| 13 | whoCanLeaveGroup | 任意 | `NONE_CAN_LEAVE` | `NONE_CAN_LEAVE` | `NONE_CAN_LEAVE` | メンバーが自分で退会不可 |
| 14 | whoCanModerateMembers | 任意 | `NONE` | `NONE` | `NONE` | |
| 15 | whoCanModerateContent | 任意 | `NONE` | `NONE` | `NONE` | |
| 16 | whoCanAssistContent | 任意 | `NONE` | `NONE` | `NONE` | |
| 17 | enableCollaborativeInbox | 任意 | `FALSE` | `FALSE` | `FALSE` | 共同トレイ無効 |
| 18 | whoCanDiscoverGroup | 任意 | `ALL_MEMBERS_CAN_DISCOVER` | `ALL_MEMBERS_CAN_DISCOVER` | `ALL_MEMBERS_CAN_DISCOVER` | |
| 19 | defaultSender | 任意 | 空白 | 空白 | 空白 | |

> デフォルト値が設定されている項目は、AppSheetフォーム上でグループ種別に応じたデフォルト値を初期表示する。操作者が必要に応じて変更できる。
> グループ種別の判定は、メールアドレスのプレフィックスにより自動的に判定する。`rk-` プレフィックス → rakumo用、`ml-` プレフィックス → メーリングリスト用、それ以外 → ドライブ用のデフォルト値を適用する。
...
> **GWSグループの削除方式：** Google Workspaceではグループの論理削除（停止状態）は不可であり、削除は物理削除のみとなる。アカウントについてはsuspended（停止）状態への変更で論理削除として扱う。
...
| Googleアカウント登録（定期/予約実行） | 4.2 Googleアカウント登録機能 | ユーザーごとに予定実行日付で指定した日時 | 日付未設定の場合は即時実行。日付形式は「YYYY/MM/DD hh」（JST） |
...
| 日時指定形式 | 「YYYY/MM/DD hh」形式で指定（1時間単位）。例：2026/04/01 09 |
| 設定変更 | 実行前であれば任意のタイミングで変更可能 |

**日時の指定と表示方法（確定 4/16追加）：**

| 項目 | 仕様 |
|------|------|
| 日時指定のタイムゾーン | 予定実行・定期実行の日付と時刻の指定はJST（日本標準時）で行う |
| ログ表示のタイムゾーン | 実行ログの日時表示はJST（日本標準時）で表示する |

> システム内部での時刻管理はUTCで行うが、ユーザーへの入力・表示はすべてJSTに変換して扱う。

**ユーザーごとの予定実行（確定 3/26）：**

| 項目 | 仕様 |
|------|------|
| 設定単位 | ユーザーごとに予定実行の日付を設定する |
| 日付未設定の場合 | 日付を空欄にした場合は即時反映とする |

> 例：新入社員の入社日を事前に登録しておき、入社日当日にGoogleアカウントを自動作成することが可能。日付を空欄にした場合は登録操作と同時に即時処理される。
...
> **ライセンスフラグのデフォルト値：** ライセンスフラグ（Calendar Enabled〜Attendance Enabled）のデフォルト値は `1`（割り当て）とする。新規アカウント作成時は全ライセンスを割り当てた状態で登録し、個別に変更する。
...
| 4 | グループ種別 | drive（ドライブ用）/ mailing（メーリングリスト用、プレフィックス: `ml-`）/ rakumo（rakumo用、プレフィックス: `rk-`）/ other |
...
| 可用性 | 冗長構成 | なし |
| 性能 | 処理件数 | 10,000アカウント＋10,000グループ規模のデータを処理できること |
| 性能 | バッチ処理時間 | 10,000アカウント＋10,000グループの一括処理を1時間以内に完了すること（目標値） |
| 性能 | バックエンド（Cloud Run） | 10,000件規模のCSV処理・API呼び出しに耐えられるよう、メモリ・CPU・タイムアウト値を適切に設定すること |
| 性能 | データベース（Cloud SQL） | 10,000件規模のレコードを扱える容量・接続数を確保すること。インスタンスタイプは処理量に応じて選定する |
| 性能 | バッチ処理 | Google Workspace API のレート制限（QPS）を考慮し、一括処理時はBatch APIリクエストを使用する。具体的な実装方式は設計フェーズで決定する |
...
| 信頼性 | 重複登録時の動作 | GWS（Google Workspace）への重複登録：GWSへの反映時に重複チェックを行い、登録エラーとする（AppSheet入力時点では重複チェックを行わない）。rakumoへの重複登録：後から登録された情報で上書きする |
...
| グループ種別 | 命名規則 | 例 | 用途 |
|------------|---------|-----|------|
| Googleドライブ用 | **命名規則なし** | ― | Googleドライブのアクセス権限管理（フラット構造） |
| メーリングリスト用 | `ml-` プレフィックス | `ml-xxxx@so.tohoyk.co.jp` | メーリングリスト |
| rakumoサービス用 | `rk-` プレフィックス + 番号 | `rk-10000@so.tohoyk.co.jp` | rakumoサービスの利用グループ（階層構造） |

> **注意：** Googleドライブ用グループは命名規則を設けない。メーリングリスト用は `ml-` プレフィックスを付与する。rakumo用グループは `rk-` に続けて番号を付与する形式とする（例：`rk-10000@so.tohoyk.co.jp`）。
>
> **重要：** グループを削除して新規作成すると、Googleドライブのアクセス権を再設定する必要が生じる。グループ情報を変更する場合は**削除→新規作成ではなく名称変更を推奨**する。

#### 更新対象グループの管理

本システムが同期・更新するグループの種別は、**Cloud SQLの設定テーブルで管理**し（AppSheetから変更可能）、コードを変更せずに更新対象グループを追加・変更できるようにする。

- **初期対象：Googleドライブ用・メーリングリスト用（`ml-`）・rakumoサービス用（`rk-`）の3種別**
- 設定変更のみで新しいグループ種別・プレフィックスを対象に追加できること
- 複数種別のグループを同時に一括更新できること

### 8.2 人事異動時の作業順序

人事異動・組織改編時の操作（CSV 一括アップロード・AppSheet 個別操作のいずれも）は、以下の順序で実施する。

1. グループ情報の変更（グループの追加・変更・削除）
2. グループメンバー情報の更新
3. ユーザー情報の変更
...
CSV一括変更を行う場合は、以下の順序でCSVを作成・適用する。

1. グループCSVの作成と適用
2. グループメンバーCSVの作成と適用
3. ユーザーCSVの作成と適用

> 全体処理の流れ：CSV作成 → GWS（Google Workspace）更新 → rakumo更新（Google同期 + コンタクトCSVアップロード）

#### グループアドレスの登録・更新

グループアドレスを登録・更新する場合、3種類のプレフィックス（ドライブ用、メーリングリスト: `ml-`、rakumo: `rk-`）の複数グループを同時に更新することができる。
...
| 変更種別 | ○ | 追加 / 変更 / 削除 |
| password | △ | パスワード（追加時は必須。8〜100文字のASCII文字） |
| suspended | ― | ユーザーの一時停止（TRUE/FALSE） |
| changePasswordAtNextLogin | ― | 次回ログイン時パスワード変更（TRUE/FALSE）。デフォルト: TRUE |
| includeInGlobalAddressList | ― | GALへの表示（TRUE/FALSE）。デフォルト: TRUE |
| orgUnitPath | ― | 所属する親組織のフルパス（例: /営業部） |
| recoveryEmail | ― | アカウント再設定用メールアドレス |
| recoveryPhone | ― | アカウント復元用スマートフォン（E.164形式） |
| 姓 | △ | ユーザーの姓（追加・変更時は必須） |
| 名 | △ | ユーザーの名（追加・変更時は必須） |
| 姓よみ | ― | 姓のふりがな |
| 名よみ | ― | 名のふりがな |
...
> 出力レベルは「サマリ」と「詳細」を切り替えられること。デフォルトは「サマリ」とする。

> **出力レベルの設定方法：** ログ出力レベル（サマリ/詳細）はAppSheetのビュー（設定画面）から変更可能とする。設定値はCloud SQLの設定テーブルに保存する。

**日時の表示方法（確定 4/16追加）：**

| 項目 | 仕様 |
|------|------|
| ログの実行日時表示 | JST（日本標準時）で表示する |
| 内部保存形式 | UTC（協定世界時）で保存し、表示時にJSTへ変換する |

> 実行ログの日時はすべてJST表示に統一する。Cloud Loggingのログもタイムゾーン表示をJSTに設定して確認できるようにする。
...
**ローテーション実行仕様：**
- 1日1回、AM 1:00（JST）にバックエンド処理（Cloud Run）にて、設定期間を超過したログレコードおよびファイルを一括削除する。実行時刻はCloud SQLの設定テーブルに格納し、AppSheetから変更可能とする。
